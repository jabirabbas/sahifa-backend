(function ($) {
  ...
  DayScheduleSelector.prototype.renderHeader = function () {
    ...
    this.$el.find('.schedule-header').html('<tr><th></th>' + html + '<th></th></tr>');
  };
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(list, function (i, d) {
      var daysInARow = $.map(new Array(days.length), function (_, i) {
        return '<td class="time-slot" data-time="' + d + '" data-day="sun"></td>'
      }).join();

      $el.append('<tr><td class="time-label">RU ' + d + '</td>' + daysInARow + '<td data-row="r'+d+'"></td></tr>');
    });
  };
  ...
  DayScheduleSelector.prototype.select = function ($slot) { $slot.attr('data-selected', 'selected').attr('data-disabled','disabled'); }
  ...
  function isSlotDisabled($slot) { return $slot.is('[data-disabled]'); }
  /**
  ...
  DayScheduleSelector.prototype.attachEvents = function () {
    ...
    this.$el.on('click', '.time-slot', function () {
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        if (isSlotSelected($(this)) && !isSlotDisabled($(this))) { plugin.deselect($(this)); }
        else {  // then start selecting
          if(!isSlotDisabled($(this))){
            //disallowing multi slot selection, by jabir
            plugin.$el.find('.time-slot:empty').removeAttr('data-selected'); 
        ...
        if (day == plugin.$selectingStart.data('day')) {  // if clicking on the same day column
          ...
          //plugin.$el.find('.time-slot:empty').removeAttr('data-disabled');
    ...
    this.$el.on('mouseover', '.time-slot', function () {
      ...
      if (plugin.isSelecting()) {  // if we are in selecting mode
        ...
        $slots.slice(start, end + 1).not('[data-disabled]').attr('data-selecting', 'selecting');
      }
    });
  };
  ...
  DayScheduleSelector.prototype.serialize = function () {
    ...
    //$.each(this.options.days, function (_, v) {
      var start, end;
      start = end = false; selections = [];
      plugin.$el.find(".time-slot[data-day='sun']").each(function () {
        if (isSlotSelected($(this)) && !isSlotDisabled($(this)) && !start) { start = $(this).data('time'); }
        else if (!isSlotSelected($(this)) && !!start) {
          end = $(this).data('time');
          selections.push([end+1, start]);
          start = end = false;
        }
      });
    //})
  ...
  DayScheduleSelector.prototype.deserialize = function (schedule) {
    ...
    $.each(schedule, function(d, ds) {
      ...
      $.each(ds, function(_, s) {
        ...
        plugin.$el.find('[data-row=r'+s[0]+']').html('<i style="cursor:pointer" title="Detach Server" class="fa fa-times text-danger"></i>');
      })
    });
  };
  ...
  function generateDates(start, end, interval) {
    var numOfRows = end;//Math.ceil(timeDiff(start, end) / interval);
    return $.map(new Array(numOfRows), function (_, i) {
      // need a dummy date to utilize the Date object
      return i;//new Date(new Date(2000, 0, 1, start.split(':')[0], start.split(':')[1]).getTime() + i * interval * 60000);
    });
  }
